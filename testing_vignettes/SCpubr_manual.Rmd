...
cells.plot <- colnames(sample[, !(sample$seurat_clusters %in% c("2", "5", "8"))])
SCpubr::do_FeaturePlot(sample, cells.highlight = cells.plot, features = c("CD14"), plot.title = "My very important features", ncol = 2)
```
...
SCpubr::do_FeaturePlot(sample, idents.highlight = c("2", "5", "8"), features = c("CD14"), plot.title = "My very important features", ncol = 2)
```

## Modifying the ends of the color scale.
Sometimes, one might want to delimit the range of the color scale. This might be relevant when plotting a variable for which we are only interested in positive values. This is the case of the enrichment scores computed using `Seurat::AddModuleScore()`. Sometimes, they fall into the negative values, potentially meaning that these cells, for the particular list of genes provided, not only they are not enriched in them but actually underexpressed with regards a control set. Delimiting the range of the color scale can be achieved using the parameters `scale.begin` and `scale.end`:

```{r, fig.cap = SCpubr FeaturePlot delimiting the range}
# Compute enrichment scores for CD14 and LYZ, known markers for CD14+ monocytes.
sample <- Seurat::AddModuleScore(sample, features = list(c("CD14", "LYZ")), name = "Monocyte_signature")
# Plot. The enrichment is always stored as metadata with the name provided plus a "1" afterwards.
p1 <- SCpubr::do_FeaturePlot(sample, features = "CD14")
p2 <- SCpubr::do_FeaturePlot(sample, features = "LYZ")
p3 <- SCpubr::do_FeaturePlot(sample, features = "Monocyte_signature1", plot.title = "Monocyte signature")
p4 <- SCpubr::do_FeaturePlot(sample, features = "Monocyte_signature1", plot.title = "Adjusted scale", scale.begin = 0, scale.end = 2)
